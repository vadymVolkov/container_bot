...
API_TOKEN = config.token2

WEBHOOK_HOST = config.ip
WEBHOOK_PORT = config.port1  # 443, 80, 88 or 8443 (port need to be 'open')
...
@bot.message_handler(
    func=lambda mess: "Создать новую Таблицу" == mess.text,
    content_types=['text'])
def handle_text(message):
    function.create_new_table_step1(message)

@bot.message_handler(
    func=lambda mess: commands.check_tables_list(mess),
    content_types=['text'])
def handle_text(message):
    function.select_table(message)
